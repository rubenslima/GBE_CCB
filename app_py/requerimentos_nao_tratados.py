"""
Projeto         : requerimentos não tratados
Solicitante     : Marilene
Autor           : Rubens Lima
Criado em       : 2025-10-10
Última alteração: 2025-10-10
Versão          : 1.0.01.b
Descrição       : Geração de planilha para conferência de requerimentos não tratados
Tipo            : ETL
Módulo          : Conferência CCB
Tags            : requerimentos não tratados
ID              : GBE.CCB.20251010.003.CLI
"""
...
where rtrim(MATRICULA)+'@'+rtrim(NumeroBeneficioINSS) in
     (select rtrim(MATRICULA)+'@'+rtrim(NumeroBeneficioINSS)
     from #passo01
    where [status] in ('DEFERIDO','EM ANALISE','ANALISADO')   )
...
select * from #passo01
    ...
    nome_arquivo = os.path.join(out_dir, f"Requerimentos_Nao_Tratados_{ts}.xlsx")
